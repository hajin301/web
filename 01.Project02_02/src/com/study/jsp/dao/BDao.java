...
public ArrayList<BDto> list3(int curPage) {
	...
	public ArrayList<BDto> BSearch(String find_field, String find_name, int curPage, HttpServletRequest request) 
	{
		...
		HttpSession session = request.getSession();
		find_field = (String)session.getAttribute("find_field");
		...
		if(find_field.equals("bTitle")) {
			...
			try {
				...
				while (resultSet.next()) {
					...
					dto4 = new BDto(bId, bName, bTitle, bContent, bDate,
							       bHit, bGroup, bStep, bIndent, Id, bMenu);
					dtos4.add(dto4);
				}
			}catch(Exception e) {
